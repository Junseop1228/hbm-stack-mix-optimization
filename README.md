...
> **v2 정정 (2026.08.12).** 외부 심사 지적을 반영해 **최종 검사의 테스터 시간을 모델에 포함**했다. 이 정정으로 기준 믹스·병목 전환점·몬테카를로 분포가 전부 변했고, **8단이 최적 믹스에서 사라졌다** — 종전 미규명이던 8단 등장은 최종 검사 시간 누락의 인공물이었다. 아울러 **DPPM 하한의 닫힌형과 층수 무의존성**, **비균일 검사 위치가 테스터 시간을 최대 68.8% 절감**한다는 결과를 새로 얻었다. 판정 근거와 처리 내역은 [`notes/12_Review_Response.md`](notes/12_Review_Response.md).

**① 설비 증설 우선순위는 품질 목표가 정한다.**
출하 품질 목표가 **425 ppm보다 느슨하면** 병목 전환점이 0.728로 고정돼 품질과 무관하다. 그보다 조이면 급증하며, **200 ppm에서는 1.287** — 테스터 캐파가 본더보다 커야 한다. 품질을 조이면 사야 할 장비가 바뀐다.
...
파라미터 9개를 삼각분포로 동시에 흔든 몬테카를로 2,000회 결과, 층수 구성은 **층당 접합 수율 x**(상관 +0.415)가, 병목 위치는 **검사시간 배수 θ = t_test/t_stack**(+0.311)이 지배한다. 반면 원가 항목은 상관 0.06 미만으로, 50% 틀려도 결론이 바뀌지 않는다.

전체 서술은 IEEE 2단으로 조판한 논문판에 있다 — **[English PDF](paper/HBM_Stack_Mix_Optimization_EN.pdf)**. 짧게 보려면 [`docs/11_Report.md`](docs/11_Report.md)(요약), 수치의 정본은 [`docs/07_Results.md`](docs/07_Results.md)다.
...
> **이 관찰은 조건부다.** 정정된 기준선에서는 두 설정의 해가 동일하며(이익 차 0.000%), 대체 현상은 **품질 제약이 비구속인 영역에서만** 나타난다. 종전에 보고했던 기회비용 수치는 φ_f = 0 조건의 값이므로 철회했다.
>
> **그리고 시험한 것은 검사 *주기*뿐이고 *위치*가 아니다.** 위치를 풀면 **상단에 몰아 검사하는 것이 균일 주기보다 테스터 시간을 최대 68.8% 절감**한다 ([`docs/07_Results.md`](docs/07_Results.md) 2.4).
...
| ~~8단 등장 메커니즘~~ | **해소.** 최종 검사 시간 누락의 인공물이었다 (v2) |
| φ_f (최종 검사 시간비) | **미확보.** 기준값을 분포 최소값으로 두고 효과를 하한으로 보고 |
| ~~비균일 검사 위치~~ | **완료.** 상단 집중이 최적 — 테스터 시간 최대 68.8% 절감 |
| ~~캐파·용량계획 문헌 서베이~~ | **완료.** 9편. "캐파 문헌에 품질이 없다"는 주장 철회 |
| 층별 x i.i.d. 가정 | **검사 완료.** L 무의존성은 δ=0 조건부 |
...
notes/      작업 기록
  ...
  12_Review_Response.md  ★ 외부 리뷰 판정과 처리 기록
