...
(위 인용은 지적 당시 원문이므로 그대로 둔다. 당시 보고값이 425 ppm 이었고,
 3라운드 M-d 로 격자를 5 ppm 으로 낮춘 뒤 정본 knee 는 410 ppm 이다.
 아래 [3] 이 그 410 이 φ_f 전 구간에서 불변임을 확인한다.)
...
def main():
    ...
    # ── (a) knee ──────────────────────────────────────────
    P("[3] knee 위치의 φ_f 의존 — 정본 410 ppm 이 φ_f 전 구간에서 불변인가")
