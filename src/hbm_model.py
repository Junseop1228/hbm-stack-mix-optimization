...
# ══════════════════════════════════════════════════════════
# 인스턴스 상수 — 단일 출처 (2026.08.12 신설)
# ══════════════════════════════════════════════════════════
# 종전에는 milp.py 가 ρ=0.8, scenarios.py 가 ρ=0.9 를 각자 정의해
# **같은 논문 안에서 두 인스턴스가 '기준 시나리오'로 보고**되고 있었다.
# 1라운드 F5(수요 정의 파일별 불일치)와 같은 유형의 재발이다.
# 인스턴스 정의는 이 블록에만 둔다. 다른 파일은 import 만 한다.

_SEC_MONTH = 30 * 24 * 3600 * 0.85     # 30일 · 가동률 85%
N_BONDER = 20                          # 본더 대수 — 미확보. 비율만 결론에 쓰인다
H_BONDER = _SEC_MONTH * N_BONDER       # 본더 가용 시간 [초]
RHO_BASE = 0.9                         # H_tester / H_bonder — 미확보
H_TESTER = H_BONDER * RHO_BASE         # 테스터 가용 시간 [초]
DPPM_CAP = 200.0                       # 출하 DPPM 상한 — 미확보, 시나리오 축
